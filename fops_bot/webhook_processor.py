...
def process_webhook_queue():
    ...
    while True:
        try:
            ...
            if message_data:
                logger.info(
                    f"Processing webhook for user: {message_data.get('user', 'unknown')}"
                )
                ...
                try:
                    ...
                    if response.status_code == 200:
                        logger.info(
                            f"Successfully sent webhook for user {payload.get('user')}"
                        )
                    else:
                        logger.error(
                            f"Failed to send webhook. Status: {response.status_code}"
                        )
